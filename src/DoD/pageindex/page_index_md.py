"""Markdown to TOC tree conversion for PageIndex."""
...
async def get_node_summary(
    node: Dict[str, Any], summary_token_threshold: int = 200, model: str | None = None
) -> str:
    """Return a node summary or raw text if short."""
...
async def generate_summaries_for_structure_md(
    structure: Any, summary_token_threshold: int, model: str | None = None
) -> Any:
    """Generate summaries for a markdown-derived structure."""
...
def extract_nodes_from_markdown(
    ...
    """Extract headers and line numbers from markdown content."""
...
def extract_node_text_content(
    node_list: List[Dict[str, Any]], markdown_lines: List[str]
) -> List[Dict[str, Any]]:
    """Attach text blocks to extracted markdown nodes."""
    all_nodes = []
...
def update_node_list_with_text_token_count(
    node_list: List[Dict[str, Any]], model: str | None = None
) -> List[Dict[str, Any]]:
    """Compute token counts for nodes including descendant content."""
...
def tree_thinning_for_index(
    ...
    """Merge small nodes into parents based on token count."""
...
def build_tree_from_nodes(node_list: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Build a nested tree from a flat node list."""
    if not node_list:
        return []
...
def clean_tree_for_output(tree_nodes: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Normalize tree nodes for output."""
    cleaned_nodes = []
...
async def md_to_tree(
    ...
    """Convert a markdown document into a TOC tree."""
    ...
    if if_add_node_summary == "yes":
        ...
        threshold = summary_token_threshold or 200
        tree_structure = await generate_summaries_for_structure_md(
            tree_structure, summary_token_threshold=threshold, model=model
        )
